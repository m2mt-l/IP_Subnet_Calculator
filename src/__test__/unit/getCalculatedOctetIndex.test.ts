...
describe("getCalculatedOctetIndex", () => {
    test("get first octet", () => {
        expect(getCalculatedOctetIndex("8")).toBe(0);
    });

    test("get second octet", () => {
        expect(getCalculatedOctetIndex("16")).toBe(1);
    });

    test("get third octet", () => {
        expect(getCalculatedOctetIndex("24")).toBe(2);
    });

    test("get fourth octet", () => {
        expect(getCalculatedOctetIndex("25")).toBe(3);
    });
});
