...
describe("getIPv4HostAddressRange test", () => {
    ...
    test("ipv4 /0 host address range", () => {
        expect(ipv4Calculator(ipv4TypeKey.hostAddressRange, subnetZeroData)).toBe(
            "0.0.0.1 - 255.255.255.254",
        );
    });

    test("ipv4 /8 host address range", () => {
        expect(ipv4Calculator(ipv4TypeKey.hostAddressRange, subnetEightData)).toBe(
            "192.0.0.1 - 192.255.255.254",
        );
    });

    test("ipv4 /16 host address range", () => {
        expect(ipv4Calculator(ipv4TypeKey.hostAddressRange, subnetSixteenData)).toBe(
            "192.168.0.1 - 192.168.255.254",
        );
    });

    test("ipv4 /24 host address range", () => {
        expect(ipv4Calculator(ipv4TypeKey.hostAddressRange, subnetTwentyFourData)).toBe(
            "192.168.0.1 - 192.168.0.254",
        );
    });

    test("ipv4 /25 host address range", () => {
        expect(ipv4Calculator(ipv4TypeKey.hostAddressRange, subnetTwentyFiveData)).toBe(
            "192.168.0.1 - 192.168.0.126",
        );
    });

    test("ipv4 /26 host address range", () => {
        expect(ipv4Calculator(ipv4TypeKey.hostAddressRange, subnetTwentySixData)).toBe(
            "192.168.0.1 - 192.168.0.62",
        );
    });

    test("ipv4 /30 host address range", () => {
        expect(ipv4Calculator(ipv4TypeKey.hostAddressRange, subnetThirtyData)).toBe(
            "192.168.0.1 - 192.168.0.2",
        );
    });

    test("ipv4 /31 host address range", () => {
        expect(ipv4Calculator(ipv4TypeKey.hostAddressRange, subnetThirtyOneData)).toBe("N/A");
    });

    test("ipv4 /32 host address range", () => {
        expect(ipv4Calculator(ipv4TypeKey.hostAddressRange, subnetThirtyTwoData)).toBe("N/A");
    });
});
